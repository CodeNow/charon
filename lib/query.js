...
function resolve(address, domainNames, cb) {
  ...
  names.forEach(function(name) {
    // TODO This method call is a stub, TJ is currently working on the real one
    // TODO change `fetchRefererInstanceByDomain` -> `whatever TJ calls it`
    user.fetchRefererInstanceByDomain(address, name, function (err, instance) {
      if (err) {
        ...
        records.push({
          name: name,
          address: instance.network.hostIP,
