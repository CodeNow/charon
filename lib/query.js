...
function resolve(address, domainNames, cb) {
  ...
  if (names.length === 0) {
    monitor.increment('charon.invalid.query');
  ...
  //  bottlenecks.
  monitor.histogram('charon.lookups.per.query', names.length);

  names.forEach(function(name) {
    monitor.increment('charon.lookup');
    var lookupTimer = monitor.timer('charon.lookup.time');
    user.fetchInternalIpForHostname(name, address, function (err, hostIP) {
      if (err) {
        monitor.increment('charon.error.lookup');
