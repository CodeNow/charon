...
module.exports = {
  /**
   * UDP Server instance used by the module.
   * @type {native-dns:UDPServer}
   */
  instance: server,
   ...
   */
  _getInternalNames: function _getInternalNames(req) {
  ...
  requestHandler: function requestHandler(req, res) {
    ...
    return Promise
      .try(function () {
        ...
        var names = this._getInternalNames(req);
        ...
        return Promise
          .map(names, function (name) {
            return apiClient.resolveName(name, address);
          })
          .then(function (records) {
            res.answer = records.map(function (record) {
              return dns.A(record);
            });
   ...
   */
  errorHandler: function errorHandler(err, buff, req, res) {
    ErrorCat.report(err);
