...
var regexFilters = process.env.FILTERED_DOMAINS
  .split(',')
  .map(function (filter) {
...
function _getInternalNames(req) {
  return req.question
    ...
    .filter(function (name) {
      return regexFilters.every(function (filter) {
  ...
  return Promise
    .try(function () {
      ...
      return Promise
        ...
        .catch(function (err) {
          // Send when the lookup failed so we do not revert to navi
          monitor.increment('query.error');
          log.error({ err: err, res: res }, 'Error encountered resolving domain names');
          res.answer = [];
    ...
    .then(function () {
      queryTimer.stop();
      res.header.aa = true;
      log.info({ res: res.header, questions: req.question }, 'Send Response');
      res.send();
    })
    .catch(function (err) {
      ErrorCat.report(err);
      monitor.increment('query.error');
      log.error({ err: err }, 'Error encountered resolving all domain names');
