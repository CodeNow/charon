...
var hostIpCachedPromise = null;
/**
 * Basically only for testing.  The cache should never need to be invalidated in production
 * @private
 */
function _clearIpCache() {
  hostIpCachedPromise = null;
}
/**
 * returns the IP address of THIS machine, whatever is running this code
 * - This promise is cached in memory so it's only ever performed once
 * @private
 */
function _getSelfHostIp() {
  if (!hostIpCachedPromise) {
    hostIpCachedPromise = Promise.try(function () {
      var netInterface = require('os').networkInterfaces();
      var ethernetInterfaces = Object.keys(netInterface).filter(function (interfaceName) {
        // Filter out any keys that are not en# or eth#
        return /^(en|eth)\d*/.test(interfaceName) && netInterface[interfaceName];
      }).map(function (interfaceName) {
        // Map out only the IPv4 interfaces
        return netInterface[interfaceName].filter(hasKeypaths({ family: 'IPv4' })).pop();
      }).filter(exists);

      // Just use the first one for now
      if (!ethernetInterfaces.length) {
        throw new Error('No external network interface found');
      }
      return ethernetInterfaces[0].address;
    })
    .then(function (ipAddress) {
      ...
      return 'http://' + ipAddress + ':4242';
    });
  }
  return hostIpCachedPromise;
}
...
function start() {
  ...
  return apiClient.login()
    .then(function () {
      ...
      // This will also cache the promise, so all future calls to _getSelfHostIp are instant
      return _getSelfHostIp();
    })
    .then(function () {
