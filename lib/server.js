...
function request(req, res) {
  ...
  query.resolve(address, names, function(err, records) {
    ...
    if (records === null) {
      monitor.counter('charon.query.refused');
...
function serverError(err, buff, req, res) {
  monitor.counter('charon.error.server');
  error(err.stack);
}
...
function socketError(err) {
  monitor.counter('charon.error.socket');
  error(err.stack);
}
